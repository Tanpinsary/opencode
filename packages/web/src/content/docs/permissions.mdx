...
Use `external_directory` to allow tool calls that touch paths outside the working directory where OpenCode was started. This applies to any tool that takes a path as input (for example `read`, `apply_patch`, `glob`, `rg`, and many `bash` commands).
...
- `edit` — all file modifications performed by `apply_patch`
- `glob` — file globbing (matches the glob pattern)
- `rg` — content search (matches the regex pattern)
